...
//					Set item da order gom so luong cua sp, gia sp da dat, va san pham do
					Item item = new Item();
					item.setQty(qty);
...
//					xet tong order cua 1sp da order vi day la AddtoCart
...
//					set session
					session.setAttribute("length_order",n); // co bao nhieu item
					session.setAttribute("order", order); // set ca order
